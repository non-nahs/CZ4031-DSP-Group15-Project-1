...
// Assume an integer is 4 bytes, a pointer is 8 bytes, and a simplified key-value pair is roughly 20 bytes
...
public:
	...
	std::vector<std::string> keys; // Keys in the node
	vector<vector<int>> values;
	std::vector<Node*> child_ptr; // Pointers to child nodes
	bool leaf; // Boolean to check if the node is a leaf
	int n; // Current number of keys
	Node* ptr2next; // Pointer to the next node

	// Node constructor
	Node(int _t, Node* _ptr2next = nullptr) {
		t = _t;
		ptr2next = _ptr2next;
		leaf = true;
